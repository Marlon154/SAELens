...
@dataclass
class BMatryoshkaSAEConfig(BatchTopKTrainingSAEConfig):
    """
    Configuration class for a balanced MatryoshkaSAE.
    """
...
class BMatryoshkaSAE(BatchTopKTrainingSAE):
    """todo
    """
...
@dataclass
class BMatryoshkaTrainingSAEConfig(BatchTopKTrainingSAEConfig):
    """
    Configuration class for training a MatryoshkaTrainingSAE.
    """

    group_sizes: List[int] = field(default_factory=lambda: [128, 512, 2048, 8196])
    level_weights: List[float] = field(default_factory=lambda: [1.0, 1.0, 1.0, 1.0])
...
class BMatryoshkaTrainingSAE(BatchTopKTrainingSAE):
    ...
    cfg: BMatryoshkaTrainingSAEConfig

    def __init__(self, cfg: BMatryoshkaTrainingSAEConfig, use_error_term: bool = False):
        super().__init__(cfg, use_error_term)
    ...
    def training_forward_pass(self, step_input: TrainStepInput) -> TrainStepOutput:
        ...
        for i, intermediate_reconstruct in enumerate(intermediate_reconstructs):
            level_l2_loss = (intermediate_reconstruct.float() - step_input.sae_in.float()).pow(2).mean()
            l2_losses = torch.cat([l2_losses, level_l2_loss.unsqueeze(0)])
            beta_m = self.cfg.level_weights[i] if i < len(self.cfg.level_weights) else 1.0
            total_l2_loss += beta_m * level_l2_loss
        ...
        total_loss = total_l2_loss + aux_losses["auxiliary_reconstruction_loss"]
